...
#[rstest]
fn test_flatten_node_singleton() {
    let results = flatten_root_node(Node::singleton("A"));
...
fn test_flatten_node_enums(#[context] context: Context, #[case] node: Node) {
    set_snapshot_suffix!("{}", context.description.unwrap());
    assert_debug_snapshot!(flatten_root_node(node));
}
...
fn test_flatten_node_lists(#[context] context: Context, #[case] node: Node) {
    set_snapshot_suffix!("{}", context.description.unwrap());
    assert_debug_snapshot!(flatten_root_node(node));
}
